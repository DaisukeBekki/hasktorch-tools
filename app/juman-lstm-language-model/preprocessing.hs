...
import qualified Text.Juman.LangModel as J --juman-tools
...
main = do
  ...
  jumanOutput <- J.processTextsByJuman jumanlogDir textFiles -- | [[JumanData]] .txtファイルをJuman++で処理
  let (kihons,hinsis) = J.buildDictionary [] $ concat jumanOutput -- | （登場した語のリスト、登場した形態素のリスト）
  J.saveDictionary dicFile $ J.DictionaryData (sortWords 3 kihons) (sortWords 0 hinsis) -- | 語のリストから辞書を作成し、dicFileに保存
