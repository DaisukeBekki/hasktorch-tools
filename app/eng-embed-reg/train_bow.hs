...
main = do
  ...
  -- | training setting
  let iter = 100::Int -- | ■■■
      hypParams = MLPHypParams device dicDim [(2048,Elu),(32,Elu),(5,Elu)] -- | ■■■
  ...
  ((trainedModel,_),losses) <- mapAccumM [1..iter] (initModel,GD) $ \epoc (model,opt) -> do
    let dists = stack (Dim 0) $ (flip map) trainingData $ \(_,comment) -> 
                  mlpLayer model $ squeezeAll $ sumDim (Dim 0) KeepDim Float $ stack (Dim 0) $ map (asTensor'' device . oneHot) $ comment
...
instance Randomizable HypParams Params where
  sample HypParams{..} = 
    Params
    <$> sample (LinearHypParams dev dict_dim emb_dim)
    <*> sample (MLPHypParams dev emb_dim hidden_dim output_dim act1 act2)
...
instance A.ToJSON AmazonReview
instance A.FromJSON AmazonReview
